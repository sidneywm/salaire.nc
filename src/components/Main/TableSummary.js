...
const TableSummary = (props) => {
  ...
  const { ruamm, retraite, chomage, ccs, retraiteAgircArrco, ceg, cet } = props.values;

  const rows = [
    createData("RUAMM Tranche 1", rates.ruamm.tranche1, ruamm.tranche1),
    ...(ruamm.tranche2 ? [createData("RUAMM Tranche 2", rates.ruamm.tranche2, ruamm.tranche2)] : []),
    ...
    createData("Retraite Agirc-Arrco Tranche 1", rates.retraiteAgircArrco.tranche1, retraiteAgircArrco.tranche1),
    ...(retraiteAgircArrco.tranche2
      ? [createData("Retraite Agirc-Arrco Tranche 2", rates.retraiteAgircArrco.tranche2, retraiteAgircArrco.tranche2)]
      : []),
    createData("CEG (Contribution d'équilibre général) Tranche 1", rates.ceg.tranche1, ceg.tranche1),
    ...(ceg.tranche2
      ? [createData("CEG (Contribution d'équilibre général) Tranche 2", rates.ceg.tranche2, ceg.tranche2)]
      : []),
    ...(cet ? [createData("CET (Contribution d'équilibre technique)", rates.cet, cet)] : []),
  ];
        ...
        <TableBody className={classes.tableBody}>
          {rows.map((row) => (
            <TableRow key={row.nom}>
              ...
              <TableCell align="center">{(row.taux * 100).toFixed(2)}</TableCell>
